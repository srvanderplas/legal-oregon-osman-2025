...
[^1]: Inconclusive responses are included in the total number of comparisons performed, but not included as errorso in the numerator.
...
The basic requirements of any valid scientific method are that it must be: - accurate, meaning the conclusion reached is the correct one, - repeatable, meaning the examiner reaches the same conclusion when presented with the same evidence, - reproducible, meaning different examiners reach the same conclusions when analyzing the same evidence.
...
. As an analogy, consider what is required for regulators such as the U.S
. Food and Drug Administration (FDA) to approve a new drug
...
Historically, the Association of Firearms and Tool Mark Examiners (AFTE) method permitted an examiner to render three subjective judgments – identification, meaning they were fired in the same gun, exclusion, meaning they were fired from different guns, and inconclusive.
The AFTE Range of Conclusions also includes three inconclusive options[^5] -

[^5]: AFTE Theory of Identification and AFTE Range of Conclusions, https://nij.ojp.gov/nij-hosted-online-training-courses/firearms-examiner-training/module-09/afte-theory-identification.
...
Similar designs were also adopted by @keislerIsolatedPairsResearch2018 and @chapnickResults3DVirtual2021. While these studies have better test problem design (e.g. open v. closed), they still have some major flaws common to almost all studies in firearms and toolmark examination: there are significant levels of participant drop-out which are not accounted for in the analysis of results[^10], participants self-select instead of being randomly selected as part of a representative sample[^11], and there is no objective assessment of the difficulty of comparisons in each study (which makes it difficult to compare studies or assess the relevance of a study to a specific case
). The treatment of inconclusive responses is also a significant issue discussed belo
w.
...
That these methods generate different error rates **for the same data** is a conundrum [@dorfmanInconclusivesErrorsError2022; @inconclusives].
...
![Table demonstrating how repeatability (and reproducibility) statistics are tabulated. Only comparisons which are the same for both evaluations are considered in the numerator (represented by blue checkmarks), while all comparisons are considered in the denominator. Overall estimates are calculated using all table cells. Conditional estimates are calculated for each row of the table separately.](repeatability-table.png){#fig-reliability-tbl width="50%"}
...
An overall repeatability or reproducibility estimate for such a table would be computed by dividing the total number of trials represented by check marks by the total number of trials in the table.
This provides a general assessment of the probability that a second evaluation matches the first.
Another quantity of interest is a conditional assessment of repeatability or reproducibility: given that the first evaluation was Y, what is the probability that the second evaluation is also Y?
This calculation considers only the entries in a single row, dividing the trials represented by a single check mark by the total number of trials in that row. 
Conditional estimates may be more useful to the court, as an examiner's decision on that evidence is known. 
Conditional estimates from black-box studies would then provide repeatability or reproducibility estimates which are tailored to the specific case, subject to the limitations of participant selection and study design discussed throughout this document. 
...
That is, more than 10% of the time, the examiner will come to a conclusion other than identification when given another look at evidence^[In Ames II, there were at least \~6 weeks between examinations].
...
While there is no legal threshold for what would be considered reliable, it is useful to assess these repeatability estimates in light of thresholds used in other areas.
...
Conversely, when an examiner's first decision is an inconclusive-B (no evidence in either direction) under the AFTE ToI, the probability that a second decision is also an inconclusive B is higher for polygonal rifling than for traditional rifling (likely because many examiners do not attempt to make source decisions on polygonally rifled evidence, either because of personal beliefs about the identifiability of such evidence or because of lab policy).
