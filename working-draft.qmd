...
I hereby declare that this statement is true to the best of my knowledge and belief, and that I understand it is made for use as evidence in court and is subject to penalty for perjury. 
This document should not be construed to represent or speak for the University of Nebraska -- Lincoln. 
...
A current version of my CV is included as an appendix of this document. 

\clearpage

# Introduction

This document outlines the fundamental research principles used to evaluate the scientific validity of a method.
What is discussed here is not new; it describes the research requirements adhered to in science-based fields.
...
What is certain is that these types of searches result in an increased probability of error even if the error rate for any single comparison is infinitesimally small. 

When taken together, the available evidence suggests that (1) visual, subjective assessment of firearms and toolmark evidence is not reliable or validated with statistically sound studies and (2) additional concerns apply when automated intelligence tools such as NIBIN are used during the evidence assessment process.
...
Multiple, high-quality randomized trials are required, each of which needs to demonstrate the safety or efficacy of the drug for the target population. 
...
[^3]: Trade journals, including the AFTE Journal, are sometimes peer reviewed, but the peers are practitioners rather than research scientists; these reviews focus on the forensic procedures but neglect to consider the design of the study and the statistical validity of any reported results.
    ...
    Research journals are not immune from this problem, but it is at least more likely that active research scientists serving as reviewers have training in statistical analysis and experimental design and will identify poor quality designs during the review process.
...
An analogy is helpful here: each study may have a few holes, much like Swiss cheese (@fig-swiss), but when multiple studies are considered together, hopefully, these holes occur in different places.
...
As a result, the total set of studies might be considered to be valid if they reach similar conclusions despite different flaws[^convergent-validity-nuance].
...
[^convergent-validity-nuance]: Developing convergent validity through multiple studies with different flaws is an exercise in nuance, expert evaluation, and time. For instance, a 1954 study showed an association between smoking and cancer by examining a cohort of British doctors[@dollMortalityDoctorsRelation1954] -- hardly a random sample of the population. However, these studies were soon backed up by other studies on different populations in different locations, conducted using different surveys and methods of experimental control, including prospective studies and twin studies in addition to cohort studies[@bestCanadianStudyMortality1961;@dollMortalityRelationSmoking1964;@hammondSmokingRelationMortality1964;@weirSmokingMortalityProspective1970;@fribergMortalityTwinsRelation1973]. While it was not ethical to conduct the gold standard randomized controlled trial to demonstrate a causal effect, eventually the accumulation of evidence from many studies led to a general consensus that smoking causes cancer, despite the lack of randomized controlled trials supporting this relationship. It is not that every study must be perfect, but rather that studies must be high quality, with large samples and good experimental designs, and that every study cannot have the same flaws or methods of experimental control. Note that this does not imply that every collection of studies with different flaws can be used to establish convergent validity.
...
In the field of medicine, for example, the National Institutes of Health (NIH) has very strict, codified  requirements that ensure that the design of validation studies meet the highest standards, and the Food and Drug Administration (FDA) regulates and ensures the validation of tests licensed for use on patients[@nationalinstitutesofhealthInclusionWomenMinorities2022].
...
Examiners did not always claim to be able to match a specific fired bullet to a specific gun.
...
These studies have two separate but related design flaws which, on their own, render the results unhelpful in understanding the performance of the method: they use multiple unknown and known samples in the same test kit, and they are “closed-set” studies, meaning examiners know that all unknown samples have a matching known source[^6],  introducing a sudoku-like logic puzzle to the study which is not present in case work.
...
Instead, examiners fill in only the matching known sample for each unknown.
This does not allow researchers to calculate the error rate for a comparison, because they cannot determine how many comparisons were performed[^7].
...
[^ames2exp]: @bajicValidationStudyAccuracy2020 is the full 127 page report of the Ames Laboratory to the FBI, comprehensively detailing data and analysis estimating accuracy, repeatability, and reproducibility inter alia of forensic firearms examinations.
    ...
    Additional publications include @monsonPlanningDesignLogistics2022, @baldwinStudyExaminerAccuracy2023, @monsonAccuracyComparisonDecisions2023, and @monson2023repeatability.

# The Current Domain-Wide Error Rate of Firearms Examination is Unknown

While research in FATM analysis has matured since the publication of the 2009 NRC and 2016 PCAST reports, there are significant and unaddressed problems with the design of the recent studies beyond the design of the test problem.
...
Again using medicine as an example, the sensitivity, or true positive rate, estimates how often the test identifies cancer when cancer is present.
...
The sensitivity and specificity combined determine the overall accuracy rate and are used by an agency such as the FDA to determine whether the test is effective.

A patient taking the test may be interested in different statistics describing the test performance, as the patient's test has a specific outcome.
...
Modern studies conducted with open-set kit designs involving a sufficiently large number of participants tend to report nonzero error rates^[@neumanBlindTestingFirearms2022 reports a zero error rate but has only 264 bullet and 265 cartridge comparisons within a single forensic lab. @keislerIsolatedPairsResearch2018 has many comparisons and reports a zero error rate and is perhaps the only modern study to report such an error rate.].
...
In studies conducted in the United States with US-trained examiners, inconclusives are overwhelmingly used for different-source evidence as opposed to same-source evidence.
...
This result may be due to the AFTE conclusion scale, lab policies about the use of inconclusives,^[Lab policies have been the root cause of other scandals in forensic evidence, as in @giannelliNorthCarolinaCrime2012a.] or the training process used for US examiners^[@inconclusives posits that the training process for US examiners may explain or partially explain the over-representation of different-source comparisons in inconclusive decisions.], but the existence of a bias in the use of inconclusives is clear. 
...
While careful handling of inconclusives in error rate calculations can result in relatively low nominal error rates, when assessing reproducibility and repeatability, inconclusives (among other factors) cannot be so easily ignored.
The discipline now has two studies that evaluate accuracy, repeatability, and reproducibility which are discussed below. 
While these studies are flawed, they do meet many of the criteria for design provided in the NRC and PCAST reports: open-set, kit-based studies where examiners can make all  possible types of errors and cannot use design information to inform their conclusions.
...
It is more likely that this effect is due to the relatively small number of comparisons in the reproducibility portion of the study^[81 comparisons for traditionally rifled, same caliber, make, and model comparisons, spread across the 5 AFTE ToI categories. Some categories (INC-A, INC-C) do not have sufficient data to estimate conditional reproducibility given the initial rating.]. 
...
In medicine, where cancer detection from medical imaging is performed by trained specialists, standards for reproducibility and repeatability vary depending on the imaging resolution and location^[Studies report reproducibility values of of 75%[@fisetRepeatabilityReproducibilityMRIbased2019], approximately 90%[@haarburgerRadiomicsFeatureReproducibility2020], 90.7%[@pesapaneInterReaderAgreementDiffusionWeighted2021], and ~95% reproducibility[@fisetRepeatabilityReproducibilityMRIbased2019] on clinically significant measurements and identifications. In studies examining the locations of clinically significant features, percent disagreement has been converted to percent agreement based on the metrics for biological significance described in the papers.]. 
...
In other scientific disciplines, these oversights may render a study unpublishable, as many journals require data availability statements or publication of data alongside the study^[@choiCompliancePrinciplesTransparency2019. This does vary significantly by discipline[@tedersooDataSharingPractices2021], but rates in all disciplines for data sharing are higher than those in FATM studies. Medical journals are more varied: national policies require sharing of protein and DNA sequence information, but sharing of clinical information is not required by policy[@huhRecentTrendsMedical2019].]. 
...
This is of particular concern in cases such as the Ames II study, where the analysis methods used are questionable[@dorfmanReAnalysisRepeatabilityReproducibility2022], and researchers are not willing to release the data upon request despite the study being funded by agencies within the federal government.
...
With a self-selected sample of participants it becomes even more critical to take steps to ensure the participants are representative of the population of interest. 
Individuals who have the interest, time, and/or lower caseloads to participate in studies may not be representative of the wider population of firearms examiners.
...
In fact, studies which differentiate between trainee and qualified examiners  often find a higher error rate among trainees[@duezDevelopmentValidationVirtual2018]; this effect is also found in medical studies[@pesapaneInterReaderAgreementDiffusionWeighted2021]. Not all potential biases are this direct: it is possible that examiners who have time to volunteer to participate in studies would tend to have lower case loads.
...
In pattern evidence, and particularly in FATM examination, there are very few outside applications of the same techniques, which leads to insularity in the discipline - participants in studies may serve as peer reviewers for another study and design and execute their own study; this helps perpetuate and normalize poor experimental design decisions.

Naturally, examiners feel a very real investment in the results of the studies they participate in, and may have a sense of collective responsibility to not provide any reason to express doubt in the accuracy or validity of a specific process, lest it be used against the entire community.
...
While the best way to address this bias would be to have FATM studies conducted by scientists outside the discipline, this is difficult to achieve: participant recruitment, effective material selection, and a host of other considerations impact the ability of outside scientists to effectively study FATM error rates. 
Blinding the study is another possible solution, in that if participants do not know they are participating in a study, it is harder for their sense of responsibility to the discipline to skew the results of the study. 
Unfortunately, blinding FATM error rate studies is a logistical nightmare, as the study materials must be added to case management systems in a way that is compatible with the policies and procedures of each separate lab, without missing contextual information providing clues that the evidence is part of a study^[Participant consent is another obstacle in blinded studies that would need to be addressed, as within-lab studies such as @neumanBlindTestingFirearms2022 may handle consent for blinded testing differently than a study involving many different forensic labs and organizations.].

As FATM studies may be influenced by examiner biases and expectations as well as sampling biases, it is particularly important that studies work to overcome these issues, perhaps starting with those which can be most easily addressed: selecting a sample of participants that is representative of the population and not solely assembled from volunteers[^vol-sample-stats]. 
...
If the study results do not generalize well to the population, then they are not helpful when the court is evaluating the validity of FATM analysis, unless the court wishes to require studies specific to the types of firearms and ammunition used in the case at hand. 


While many scientific journals and funding agencies rely on peer review to identify and correct issues with sampling bias, the review that takes place in trade journals such as the AFTE journal does not necessarily catch and correct issues with the description and presentation of study results.
...
This bias in black-box studies cannot be allowed to continue, as it fundamentally impacts the representativeness of studies and the ability to generalize study results to casework. 
...
## Intelligence Algorithms {#sec-intelligence-algs}
...
### Cognitive Bias and Intelligence Algorithms {#sec-cog-bias}
...
It is tempting to hire technicians to enter evidence into an intelligence algorithm, using the algorithm to identify promising cases for examiners to assess in the traditional manner. 
This strategy seems like a reasonable approach when systems are operating under reduced budgets or when there is a shortage of qualified examiners; unfortunately, it introduces additional cognitive biases that current black-box studies do not address. 

When examiners only see highly similar comparisons, their prior odds of an identification increase (that is, their perception of the base rate of same-source and different-source comparisons changes). 
This is even more impactful if they know that they are only examining comparisons which have already been flagged by the intelligence algorithm. 
...
Thus, in states where such strategies are in place, including Oregon, all of the problems with the domain-wide error rate are present, along with the problems introduced by the use of intelligence algorithms, but on top of those issues, the effects of the cognitive bias of examiners primarily seeing only close non-matches and matches identified by the intelligence algorithm also have unknown effects on the error rate for FATM examination. 
...
### Empirical Evidence on Intelligence Algorithms is Vital

Intelligence algorithms like NIBIN are widely used across the country, though the amount of usage varies: in some jurisdictions, these databases and algorithms are used only in cases where there is not a second set of evidence to compare against -- that is, to identify links in otherwise unlinked cases. 
In other jurisdictions, NIBIN is used to pre-screen a significant proportion of collected firearms and toolmark data, only forwarding promising cases to examiners for manual verification. 
These two situations are statistically distinct in some ways, but both are problematic: we do not know what the error rate is for intelligence algorithms, though it will certainly be impacted by the matching algorithm used, the size of the database, the quality and type of evidence in the database, and a host of other factors.

In order to begin to understand the impact of intelligence algorithms on the error rate of FATM examination, scientists would need information on intelligence algorithm error rates when evaluating evidence from black-box studies^[This is only a starting point -- as scientists examine the use of these algorithms, it is likely that additional study designs will be created to assess algorithm performance more carefully and in different ways than we assess FATM examiner performance. One benefit of algorithms is that they do not complain when asked to perform hundreds of comparisons for a study -- in this way, algorithms are fundamentally different from trained FATM examiners.]. 
In addition, each evaluation of evidence from an intelligence algorithm would need to provide basic statistical information about the number of comparisons made and the overall database size.
If a query is used to filter the comparisons made, then there should be information reported about the overall database size as well as the size of the reduced set of comparisons performed.
In order to reduce the chance of "fishing", intelligence algorithm queries should be formally documented and recorded in a way that does not allow investigators to optimize the database search to reduce the theoretical number of comparisons once a match is identified.

In addition to the intelligence algorithm error rates and information, however, we also need information about how examiners perform when evaluating close non-matches and same-source comparisons, under conditions with and without contextual bias. 
That is, we need empirical evidence about the FATM examiner error rate under likely outcomes of intelligence algorithm use. 
With this additional information, it may be possible to begin to assess the error rate when tools like NIBIN are used in different contexts. 
However, as NIBIN and similar tools are closed and highly controlled platforms, this information is currently difficult for unaffiliated researchers to acquire (and affiliated researchers would have a conflict of interest that would potentially taint any results). 
Until courts begin to require information from the institutions which support intelligence algorithms, it will be difficult for research in this area to proceed, and without that research, scientists and statisticians can only raise alarms over the likely outcome of large database searches -- that is, increased false identifications.

# Conclusion

While firearms and toolmark analysis studies have improved, the current state of the discipline is still well below thresholds of scientific validity applicable to other disciplines. 
...
Objective algorithms that evaluate the similarity of evidence do not typically utilize inconclusives, and still have error rates which are lower than examiners in head-to-head comparisons.
This indicates that inconclusives aren't essential to the process of evaluation and calls into question the AFTE Theory of Identification rating system. 

A detailed examination of one of the best (though still deeply flawed) studies to date (Ames II) even provides affirmative evidence that FATM examination does not meet the bar for scientific validity:

- **Accuracy**: FATM examiners only identify the correct source (same or different) `{r} paste(paste0(unlist(round(ames2_accuracy_csdr[,2], digits = 2)), "%"), collapse = " and ")` of the time, respectively, for cartridge cases and bullets. 

- **Repeatability**: When an examiner evaluates the same evidence again after several weeks, they will come to a different conclusion on the 5-category AFTE scale `{r} paste(ames2_cdl_repeat_tbl[6,2:3], collapse = " and ") |> str_remove_all("\\(.*?\\)")`  of the time for bullets and cartridge cases, respectively.

- **Reproducibility**: A second examiner assessing the same evidence will come to different conclusion on the 5-category AFTE scale `{r} paste(ames2_cdl_repro_tbl[6,2:3], collapse = " and ") |> str_remove_all("\\(.*?\\)")` of the time for bullets and cartridge cases, respectively.

While only a single study has enough data to produce reasonable evidence about reproducibility, there are two studies assessing repeatability, and both cast doubt on the repeatability of the method. 
Furthermore, multiple reliable studies estimate the correct source decision rate (that is, the probability that an examiner correctly identifies a same-source or different-source comparison) at  between `r paste(sprintf("%0.2f%%", 100* range(reliable_studies$CSDR, na.rm = T)), collapse = " and ")`. 
...
All current studies have serious flaws that limit their external validity and may under-estimate the error rates involved in FATM examination, but even if these issues are ignored, the existing evidence suggests there are serious problems with the repeatability and reproducibility of subjective FATM examination procedures.

The evidence from these new studies shifts status quo. 
Previously, the question was whether enough studies showed evidence of scientific validity; at this point, studies addressing reproducibility and repeatability suggest that subjective FATM analysis using the AFTE scale for identification lacks scientific validity.
At this point, using the same criteria established by PCAST for reliability in a scientific discipline, subjective firearms and toolmark examination can no longer be considered scientifically reliable because of the low rates of reproducibility.

Moving forward, the burden must be on FATM examiners, forensic labs, and accrediting agencies to show how policies and procedures have been changed to make subjective FATM examinations more objective and scientific. 
This will require conducting well-designed studies that empirically demonstrate that adoption of e.g. algorithmic aids for FATM examiners meaningfully increases reproducibility and reliability. 
Until labs and agencies have made shifts that produce demonstrated improvements in the reliability, repeatability, and reproducibility of FATM examinations, evidence from these sources cannot be considered scientifically valid. 

Moreover, in jurisdictions which use intelligence algorithms to pre-screen collected evidence before forwarding promising evidence to examiners for manual review, the situation is much more critical. 
The concerns discussed in @sec-intelligence-algs, and in particular, those mentioned in @sec-cog-bias, have a compounding effect. 
In these jurisdictions, problems with the process for evaluating FATM evidence are known and are expected to increase the as-yet-unknown domain-wide error rate. 
The statistical evaluation of intelligence algorithms for FATM evidence suggests a perfect storm: error rate estimates spanning orders of magnitude, large numbers of comparisons within an intelligence database, and probable selection of close non-matches along with same-source evidence, and examiners who expect to reach an identification decision because of the processes involved in selecting cases for manual examination. 
As a result, the statistical validity of firearms evidence evaluation in these jurisdictions is even more fraught than in jurisdictions which are not using intelligence algorithms to pre-screen comparisons.
...
study_chars |>
  mutate(year = str_extract(Study, "\\d{4}") |> as.numeric()) |>
  arrange(desc(year), Study, Type) |>
  knitr::kable()
```
:::
...
hicklin_repeatability_gun |>
  ...
  mutate(eval1 = factor(eval1, levels = cdl_tbl_opts, ordered = T)) |>
  arrange(eval1) |>
  rename("Eval 1" = eval1, "Same Source" = `same-source`) |>
