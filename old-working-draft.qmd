...
There are an impressive array of existing studies of the "validity" of firearms and toolmark (FATM) comparisons across different firearms, ammunition types, types of marks made, and even across different countries with different protocols for training and firearms and toolmark examination.
<!-- [@keislerIsolatedPairsResearch2018;@lightstonePotentialPersistenceSubclass2010; @rivaObjectiveEvaluationSubclass2017; @bunch2003comprehensive; @mayland_validation_2012; @duezDevelopmentValidationVirtual2018; @pauw2013faid; @neel2007comprehensive; @hambyWorldwideStudyBullets2019; @gouwe2008comparison; @girouxEmpiricalValidationStudy2009; @stromanEmpiricallyDeterminedFrequency2014; @lyonsIdentificationConsecutivelyManufactured2009; @mattijssenValidityReliabilityForensic2020; @smithValidationStudyBullet2016; @monsonAccuracyComparisonDecisions2023; @chapnickResults3DVirtual2021; @guyllValidityForensicCartridgecase2023; @baldwinStudyExaminerAccuracy2023]  -->
Statisticians have significant qualms with the state of error rate studies in firearms and toolmark examination.
...
Furthermore, the studies applicable to FATM examination in the United States all suffer from sampling and non-response bias that renders them unreliable for the purposes of establishing the science of firearms and toolmark examination as a reliable discipline [@khanHierarchicalBayesianNonresponse2023;@khanShiningLightForensic2023].

This article lays out some of the fundamental problems with the state of firearms and toolmark examination error rate studies.
...
I approach the problem of scientific support for firearm and toolmark analysis as a statistician who regularly conducts studies with human participants.
Statisticians are often tasked with helping other scientists design studies that draw scientifically valid conclusions about observable phenomena.
I have experience working in situations where lives hang in the balance when errors are made: nuclear power generation, scientific communication, and forensic science, among others.
...
Studies should ensure that participants can make **every type of error**, without the use of structural information that is not available during casework.
The best studies use a "kit" design that provides *sets with one unknown and one known firearm* (there may be multiple fired components from each weapon). 
A reliable study must also record the rate of participant drop-out and item non-response (leaving questions blank) and account for these issues when calculating error rate estimates.
:::

A statistical estimate is only as reliable as the data used to generate that estimate.
Here, we focus primarily on characteristics of the experiment: how experimenters recruit participants, the study design, and how participant responses are reported. 
...
In practice, this means that black-box studies should be open set studies (no guarantee that an unknown item matches any provided knowns in the set) and should involve the comparison of one standard (known sample) and one unknown sample at a time.
...
This means that experimenters must take care to provide examiners with test samples which are not shared with other  examiners in the same lab, and which may have different numbers of same-source and different-source samples.
This prevents information from "leaking" from examiner to examiner within a relatively small community.
In addition, experimenters must observe strict protocols when communicating with participants to avoid sharing information about the task beyond what is specified in the instructions.
Experiments are often "pilot tested" before the main data collection phase; there should not be any overlap between pilot test participants and participants in the main study, unlike in @hicklinAccuracyReproducibilityBullet2024 to avoid introducing a practice effect that might decrease the error rate estimate.
...
Experimenters should provide data at the individual level if it is possible to do so without identifying participants. 
If this is not possible, data should be provided at the lowest level of aggregation which maintains participant anonymity.
It is often possible to include individual comparison answers for each participant while separating the demographic question answers and reporting these in aggregate. 
...
One of the primary concerns with error rates provided by studies that would be considered "well-designed" under PCAST principles is that even well designed, well-executed studies cannot compensate for sampling bias in the participant pool.
**Sampling bias** occurs when some members of the population are more likely to be selected to be part of the experiment. 
The results of a biased experiment will more closely resemble these members of the population.
...
There are, of course, many variations between a simple random sample and a full census that are used in other fields to get a representative sample from a difficult-to-quantify population^[@morrisExplorationMusicianResilience2013, for instance, describes a method for sampling jazz musicians after Hurricane Katrina in New Orleans, based on the premise that you're not a jazz musician if you don't know at least a couple of other jazz musicians; this type of peer-to-peer sampling method (snowball sampling) is particularly useful in cases where there is no sampling frame which lists all of the members of a specific population.].
In cases where a full census is not completed, experimenters must use the defined selection criteria to argue that the study sample is representative across important aspects of the population.
...
In all of the remaining black-box studies, many of the potential differences between volunteers and the general population likely have no impact on estimated error rates, but there are also several potential lurking covariates that would meaningfully affect the error rates estimated by the studies.
...
When participants are randomly selected from the population, these effects may still exist due to sampling variability, but over many studies, small biases in any direction will cancel out, and even in single studies, we can quantify (and bound) the effect of sampling variability. 
...
Under these circumstances, it is particularly important to select a sample of participants which is representative of the population and not solely assembled from volunteers. 
...
Currently, there is no simple way to randomly sample all qualified examiners and compel them to participate, in part because research participation must be voluntary in the United States. [^7] 
...
Polling AFTE members might reach a set of participants who are more invested in the discipline. 
Individuals who have the time and/or lower caseloads to participate in studies might not be representative of the wider population of firearms examiners in part because these are things that were not addressed by study authors when describing the participant selection in the study.
...
Generalizing error rate studies to casework requires a broad range of studies that include both studies involving consecutively manufactured firearms and studies of multiple non-consecutively manufactured firearms. 
...
To do otherwise risks continuing the status quo of studies which produce artificially low error rates through a combination of well-marking firearms and ammunition, inconclusive results, and examiner non response [@gutierrezBowlingBumperRails2023]. 
...
::: {.callout-note icon=false}
#### Summary
Studies of error rates in firearms examination often do not report drop-out and non response rates, which has the potential to artificially reduce error rate estimates as examiners fail to answer questions about difficult comparisons. 
Without accurate reporting of drop-out and non response rates over the course of a study, it is impossible to determine how much of a threat these issues are to the overall validity of error rate estimates produced by the studies. 
:::
...
There are many statistical methods used to produce valid estimates even in the face of missing data and non-response bias [^9], and @khanHierarchicalBayesianNonresponse2023 use statistical modeling to explore the effect of drop-out and item non response on error rates.
...
Unfortunately, statisticians do not have sufficient data to adjust the estimates based on these issues, because statistical corrections for non response in FATM error rate studies are not nearly as well established as political polling.
...
<!-- In most studies we have reviewed, the limitations due to non response and drop-out bias are not acknowledged. -->
<!-- No study utilizes common statistical methods for assessing the impact of non response and drop-out bias [@woodAreMissingOutcome2004]. -->
...
In summary, item non response and drop-out bias are significant problems in current error rate studies.
...
**Reliability** is a measure of how often process produces a result that matches reality. 
Reliability is assessed using a number of different quantities that describe different types of errors.
The simplest single-number summary of reliability is the **correct source decision rate** (CSDR), which measures the proportion of the time a FATM examiner correctly identifies whether the samples came from same or different sources. 
...
As firearms and toolmark examination is a subjective process, this measure tells us about the variability present in an examination because of the subjectivity. 
Objective, deterministic algorithms would be expected to come to the same conclusion each time, but human examiners may not. 
...
Reproducibility should be lower than repeatability, because it includes additional sources of variation: different examiners, equipment, and procedures. 


**All three measures** must be established experimentally, and must be high enough to meet the criteria for scientific validity. 
While many studies have been conducted to assess reliability, to date only two studies also address repeatability and reproducibility. 
...
Multiple different error rates can be calculated from a validation study when the data are laid out in this simple and direct format.
...
Additionally, studies which use different firearms, ammunition, or other factors may provide individual tables for each design element. 
While more complicated studies are common, the results from subgroup analyses can still be assessed using a simple table of results like @tbl-error-reference.
...
@texasforensicsciencecenterFinalReportComplaint2024 identifies the issue of false-positive and false-negative error rates with inconclusives and recommends a comprehensive approach that describes both the error rate of conclusive decisions as well as the rate of inconclusive decisions, using a set of 2x2 and 3x3 tables. 
...
This is a subject of widespread, ongoing discussion within both the examiner community and the academic research community; if you ask three academics familiar with this issue for an opinion, you might get four different opinions within the scope of a short discussion [@drorCannotDecideFine2019; @dorfmanInconclusivesErrorsError2022; @guyllValidityForensicCartridgecase2023; @biedermannAreInconclusiveDecisions2019;@texasforensicsciencecenterFinalReportComplaint2024].
...
This problem is sufficiently well known that  the Texas Forensic Science Commission has both addressed it with relevant examples [pg 28, @texasforensicsciencecenterFinalReportComplaint2024] and 
...
Ideally, this randomness is extremely small: a deterministic computer procedure might fail to produce the same results 0.00001% of the time, but this is an acceptably small level of inconsistency. 
...
## Acceptable Thresholds for Scientific Validity

Legal standards for evaluation of evidence^[Frye, Daubert, and jurisdictional precedent modifying either decision] lack quantitative thresholds for acceptability. 
Even the NRC and PCAST reports did not specify a threshold at which evidence would be considered sufficiently reliable, repeatable, or reproducible. 
In order to evaluate black box studies according to these metrics, then, it is useful to consider standards for evidence in other fields. 

The three characteristics necessary for scientific validity of an evidence evaluation process have an analogous counterpart in industrial statistics, where engineers and scientists often conduct Gage R&R (Repeatability & Reproducibility) studies. 
The name is somewhat of a misnomer, as these studies also consider reliability, though in these studies it is often called effectiveness or accuracy for historical reasons. 
In Gage R&R studies, a measurement method is considered acceptable when all three metrics exceed 90% (that is, when mismatches are $<10\%$) [@zanobiniRepeatabilityReproducibilityTechniques2016]. 
These requirements are variable [@soaresGageRRStudies2022] depending on the application and context of the study, and the consequences of measurement variation. 
For instance, acceptable R&R thresholds in fields such as nuclear and aviation should be higher than those for more general manufacturing, where the consequences of variability may be lower.  

While it is not possible to demonstrate the use of Gage R&R analysis methods for black-box studies because the individual trial data has not been made available for a study assessing reliability, repeatability, and reproducibility, the structure of the data would be compatible with an this type of analysis^[SV reached out to Robert Thompson at NIST to inquire about the possibility of getting the data from @hicklinAccuracyReproducibilityBullet2024, but has not received a response.] 
...
ames2_repeatability_id <- ames2_repeatability |>
...
ames2_repeatability_all <- ames2_repeatability |>
  filter(!eval1 %in% c("US", "Other") & !eval2 %in% c("US", "Other")) |>
...
ames2_repeatability_sep <- ames2_repeatability |>
  filter(!eval1 %in% c("US", "Other") & !eval2 %in% c("US", "Other")) |>
  mutate(same = eval1 == eval2) |>
  group_by(Type) |>
  summarize(Prop = 1 - sum(same * Count)/sum(Count))
...
ames2_reproducibility_id <- ames2_reproducibility |>
  filter(!eval1 %in% c("US", "Other") & !eval2 %in% c("US", "Other")) |>
  filter(eval1 == "ID") |>
  mutate(alsoID = eval2 == "ID") |>
  group_by(Type) |>
...
ames2_reproducibility_all <- ames2_reproducibility |>
  filter(!eval1 %in% c("US", "Other") & !eval2 %in% c("US", "Other")) |>
  mutate(same = eval1 == eval2) |>
  group_by(Type) |>
  summarize(Prop = 1 - sum(same * Count)/sum(Count))  
...
# Assessing Reliability, Repeatability, and Reproducibility

At this time, there are two different studies, conducted by different research groups, which assess the reliability, repeatability, and reproducibility of firearms and toolmark assessment. 
Both studies are open-set kit studies, and while there are significant differences in the experimental design of each study as well as the number of factors examined by each study, 

## Ames II {#sec-ames2-demo}
...
In particular, the drop-out and non response rates in Ames II are high enough to cast doubt on the statistical estimates of error rates produced by the study.

Acknowledging these flaws, the Ames II study showed:

- **Reliability**: The CSDR for bullets and cartridges are `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_csdr$csdr[1], ames2_reliability_csdr$csdr[2])`, respectively (higher is better, calculated from tables in  @monsonAccuracyComparisonDecisions2023). Examiners identify the correct source for bullet comparisons less than 50% of the time, which is worse than guessing. 

- **Repeatability**: With at least 6 weeks between evaluations, for both bullets and cartridge cases,
    - If the first evaluation is an identification, at least `{r} sprintf("%.2f%%", 100*ames2_repeatability_id)` of the time the examiner will come to a conclusion other than identification. 
    - Across all 5 AFTE categories, an examiner will come to a different conclusion at least `{r} sprintf("%.2f%%", 100*ames2_repeatability_all)`  of the time.

- **Reproducibility**: 
    - If the first examiner makes an identification, a second examiner will come to a different conclusion `{r} sprintf("%.2f%%  and %.2f%%", ames2_reproducibility_id$Prop[1], ames2_reproducibility_id$Prop[2])` of the time for bullets and cartridge cases, respectively.
    - Across all 5 AFTE categories, a second examiner will come to different conclusion on the 5-category AFTE scale `{r} sprintf("%.2f%%  and %.2f%%", ames2_reproducibility_all$Prop[1], ames2_reproducibility_all$Prop[2])` of the time for bullets and cartridge cases, respectively.
...
PCAST cited the Ames I study as being sufficient to estimate reliability, overlooking concerns with the drop-out rate, non response rate, and sampling methodology present in the study. 
These concerns have a large effect on the potential estimated error rate [@khanHierarchicalBayesianNonresponse2023], and as a result, many statisticians disagree with the PCAST report's evaluation of Ames I as a reliable study, not primarily because of its design, but because of its sampling, execution, analysis, and incomplete reporting of study results.
...
If we compute the drop-out rate as those who completed at least one round but did not complete all six rounds, then we would get `{r} ames2_first_to_last_round`, which is even more alarming (and does not account for those who dropped out after seeing the first test packet). 
Of those who were initially recruited, only `{r} ames2_recruited_completed_all` completed all six rounds of the study.
...
In addition to the participant drop-out rate, there are some instances of item non response as well: @monson2023repeatability reports that some examiners did not complete bullet comparisons but did complete cartridge case comparisons. 
Using the information available to us, we can come up with an approximate upper bound for the error estimate by assuming that any participant and item non-responses would have been incorrect (obviously, this will produce an inflated error estimate, but it is illustrative to show the magnitude of the problem). 
Under this assumption (that all non-responses would have been incorrect, and that all responses were correct), error rates could be as high as `{r} ames2_bullet_nonresponse` for bullets and `{r} ames2_cartridge_nonresponse` for cartridge cases (using the total number of sets analyzed in all rounds reported in Table 1 of @monson2023repeatability, compared with 256 participants $\times$ 15 comparisons per kit $\times$ 6 rounds of the study). 
This upper estimate is actually not a complete upper bound: another type of non-response is the failure to make a conclusive decision (e.g. one of the 3 levels of inconclusive allowed by the study); due to the way comparisons are reported, it is incredibly difficult to determine an upper bound for the error rate if one counts inconclusives as either omitted answers or as incorrect over all six rounds of the study. 
...
We know that it is most likely somewhere between 0 and `{r} ames2_overall_nonresponse`, but we cannot provide statistically valid assurances for a specific number within this interval. 
...
@monsonAccuracyComparisonDecisions2023 reports that in the first round, between `{r} sprintf("%0.2f%%", min(ames2_reliability_us$comparison_pct))` and `{r} sprintf("%0.2f%%", max(ames2_reliability_us$comparison_pct))` of comparisons were labeled as unsuitable.
...
However, @monson2023repeatability reports that examiners evaluated suitability differently `{r} sprintf("%0.2f%%", ames2_repeatability_us$Conflict)` of the time when examining the same evidence (computed across ground truth and evidence type). 
...
When different examiners evaluated the same comparisons ("reproducibility"), the rate of disagreement across ground truth and evidence type was `{r} sprintf("%0.2f%%", ames2_reproducibility_us$Conflict)`, which is slightly higher than the repeatability estimate, which would be expected given the variability in examiner standards and protocol across labs. 
...
- Correct Source Decision Rate: The number of comparisons which are correctly identified (for same source) or eliminated (for different source), divided by the total number of comparisons. @monsonAccuracyComparisonDecisions2023 reports the CSDR for bullets and cartridges as `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_csdr$csdr[1], ames2_reliability_csdr$csdr[2])`, respectively. Higher values indicate better methods. Note that a CSDR under 50% is worse than random chance for a choice between two outcomes (same or different source). 

- Sensitivity and Specificity:

  - Sensitivity: Ability to correctly identify same-source evidence. Strictly calculated as the number of same-source identifications divided by the total number of same source comparisons (e.g. inconclusives are not consistent with ground truth). Under this definition, the Ames II accuracy study obtained sensitivity rates for bullets and cartridges of `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_sens$rate[1], ames2_reliability_sens$rate[2])`, respectively. Higher values are better. 

  - Specificity: Ability to correctly identify different-source evidence, calculated as the number of different source eliminations divided by the total number of different source comparisons. Under this definition, the Ames II accuracy study observed specificity rates for bullets and cartridges of `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_spec$rate[1], ames2_reliability_spec$rate[2])`, respectively.  Higher values are better.


- False positive rate (FPR) and False negative rate (FNR):

  - FPR: Probability of making an identification when examining a different-source comparison. Under this definition, @monsonAccuracyComparisonDecisions2023 observed false positive rates for bullets and cartridges of `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_fp$rate[1], ames2_reliability_fp$rate[2])`, respectively. Lower values are better. 
  
  - FNR: Probability of making an elimination when examining a same-source comparison. Under this definition, the Ames II accuracy study observed false negative rates for bullets and cartridges of `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_fn$rate[1], ames2_reliability_fn$rate[2])`, respectively.  Lower values are better. 
  
- Positive predictive value (PPV) and negative predictive value (NPV):

  - PPV: Probability that when an examiner makes an identification, the comparison is same-source, calculated as the number of same-source identifications divided by the total number of identifications.  Under this definition, examiners in the Ames II accuracy study making an identification have a `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_ppv$rate[1], ames2_reliability_ppv$rate[2])` chance that the comparison is same-source for bullets and cartridges, respectively. Higher values are better. 
  
  
  - NPV: Probability that when an examiner makes an elimination, the comparison is different-source, calculated as the number of different-source eliminations divided by the total number of eliminations.  Under this definition, examiners in the Ames II accuracy study making an elimination have a `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_npv$rate[1], ames2_reliability_npv$rate[2])` chance that the comparison is different-source comparison for bullets and cartridges, respectively. Higher values are better. 
...
```{r}
#| label: tbl-ames2-conditional-reproducibility
...
Forensic algorithms should be open-source, benchmarked on public data sets for comparison purposes, and carefully validated before use in casework.
...
In more complicated cases, raw similarity scores may be compared to previously generated comparisons to assess whether a score is more likely to come from a same- or different-source comparison ^[An example of this can be seen in Figure 4 of @proficiencytestreviewadhoccommitteeReportAssociationFirearm2024]. 
This distinction is one reason why statisticians tend to be wary of the use of screening algorithms relative to the use of evaluation algorithms, even when both involve databases: databases of comparison results are categorically different than databases of exemplars used for matching.
...
Our heuristic approach to probabilities is to latch onto the most probable outcome and attach more certainty to that outcome; as a result, we are particularly bad at evaluating the risk of high-consequence, low-probability outcomes. 
...
As statisticians we deal with probabilities on a daily basis.
...
In a recent proficiency test produced by CTS, domestic FATM examiners had an error rate of at least 9.2% (the full set of responses is not provided in the AFTE report), while the overall participant error rate was 21.8%. 
This test was described by an early participant as "more similar to casework" than typical proficiency tests, indicating that proficiency tests do not usually match the difficulty of casework. 
:::

A common criticism of black-box studies from the examiner perspective is that these studies do not accurately mimic procedures used in case work. 
In designed experiments, statisticians often want to control extraneous variability, such as different lab protocols for independent verification, and restrict study comparisons to only a single examiner.
This is completely reasonable, if the goal is to establish the error rate of the comparison method in isolation; additional tests can then be used to establish the error rate of the process (or researchers can model that statistically with additional assumptions of statistical independence). 
That is, black-box studies are intended to establish the accuracy of a method, rather than a lab or an individual.

Proficiency testing, on the other hand, is designed to establish the accuracy of an examiner - to provide some assurance that each individual or lab "knows their stuff".
The results of a series of proficiency tests might be offered as evidence that an examiner has a low error rate, or the results of a set of examiners in a lab might be used to provide support for the ability of the lab to correctly make source decisions as a group.
Proficiency tests can be used to support accreditation, along with audits of processes and procedures, education, and training requirements.
Examiners may be required to complete proficiency tests as part of their employment, but independent individuals can also register for the test; it is not required that participants be affiliated with an accredited organization. 

Recently, CTS, a testing agency, produced and conducted a proficiency test (23-5262) that had a higher than expected false-positive rate. 
Of the four comparisons provided in the test, none were a match to the reference ammunition: three questioned items were from the same type of conventionally-rifled weapon, while a fourth item was from a weapon with polygonal rifling ^[Polygonal rifling is much harder, if not impossible, to use in examinations]. 
The class-characteristic mismatch between traditional and polygonal rifling was intended to allow examiners in labs which do not allow elimination on individual characteristics to make an elimination on one of the four comparisons.

In predistribution testing, no false positive errors were reported (8 of the 9 evaluations that shared class characteristics were inconclusive, with one elimination). 
One participant stated "that it was the most difficult proficiency test they have ever received, but that it was more similar to casework and further observed that there could be an increase in inconclusive results." ^[@proficiencytestreviewadhoccommitteeReportAssociationFirearm2024, page 4.]
This indicates that proficiency tests are typically less difficult than casework, at least according to this examiner.
While it would be ideal to have a quantitative indicator of the difficulty level, or some objective standard to compare against, we do at least have some qualitative indication that this test might be more representative of casework.
Based on this comment, there are some indications that the results from this proficiency test may have higher external validity, at least on the comparison difficulty metric, than many of the black-box studies that are published.

Another participant mentioned limited reproduction of individual characteristics in the known bullets, making identification challenging; this may indicate an expectation that proficiency tests include at least one identification in the set. 
This prior expectation may have impacted the results, but also illustrates a problem with unblinded testing: participants know they are being tested, have expectations about the test composition, and conform to those expectations -- an example of the Hawthorne effect in action.
This comment might be expected to reduce the external validity of the test somewhat, as the expectations in this case would not translate to casework. 

The results of the proficiency testing indicate that of the 280 examiners participating in this test, 21.8% (61 of 280) made at least one false identification; this is a significant change from the overall 1.1% rate of false positive errors on proficiency tests between 1978 and 1991 (admittedly, these estimates are dated). 
AFTE's analysis of the results broken down by lab accreditation status and location are informative, but even among domestic labs, at least 9.2% of examiners made at least one false positive error, which is an alarming error rate.
Automated analyses of test samples using 3D digital microscopy suggest that there was no support for same-source determinations when using objective tools; this  held true even for known-to-known inter-comparisons, indicating that the firearm used to generate the known samples may not have marked consistently.
Nonetheless, it is clear that subjective visual examinations by examiners performed poorly relative to two different manufacturers' objective tests.
While there are many other conclusions which can be drawn about the difficulty and design of this particular proficiency test, it provides additional evidence that the current state of subjective, visual examination by examiners is insufficient relative to the objective, statistically validated algorithms developed in the wake of the PCAST report in 2016.  

We can conclude that while proficiency testing is not equivalent to a designed study in terms of validity, this particular proficiency test at least had a difficulty level which was seen as more consistent with casework. 
While examiner expectations that proficiency tests will contain an identification may have artificially inflated the error rate, this conformity to prior expectations demonstrates that the subjective visual comparisons examiners rely upon are prone to bias in ways that can significantly impact the reliability of these visual assessments.
In addition, objective, algorithmic assessments seem to have avoided the false positive errors associated with subjective visual examination.
Taken together with the results of the designed Ames II study, these results cast serious doubt on the scientific validity of visual comparisons of firearms and toolmark evidence and reinforce PCAST's conclusion that objective, validated tools are necessary to support FATM examiners and increase the overall reliability of the discipline.
...
-   Low drop-out rates, or statistical methods for assessing the impact of drop-out rates and non-response bias on the error estimates
...
-   Difficulty of comparisons similar to that present in case work
...
- **Reliability**: FATM examiners only identify the correct source (same or different) `{r} sprintf("%.2f%% and %.2f%%", ames2_reliability_csdr$csdr[1], ames2_reliability_csdr$csdr[2])` of the time, respectively, for cartridge cases and bullets. 

- **Repeatability**: When an examiner evaluates the same evidence after at least 6 weeks, they will come to a different conclusion on the 5-category AFTE scale `{r} sprintf("%.2f%% and %.2f%%", 100*ames2_repeatability_sep$Prop[1], 100*ames2_repeatability_sep$Prop[2])`  of the time for bullets and cartridge cases, respectively.

- **Reproducibility**: A second examiner will come to different conclusion on the 5-category AFTE scale `{r} sprintf("%.2f%%  and %.2f%%", ames2_reproducibility_all$Prop[1], ames2_reproducibility_all$Prop[2])` of the time for bullets and cartridge cases, respectively.
...
We acknowledge that all current studies have serious flaws that limit their external validity and may under-estimate the error rates involved in FATM examination, but even if we do not adjust for non-response and volunteer sampling, the existing evidence suggests there are serious problems with the repeatability and reproducibility of subjective FATM examination procedures.
...
clean_studies <- clean_studies %>%
  mutate(Study = purrr::map_chr(`Study Key`, ~cite(keys = ., bib = mybib, bibpunct=c("", "",  ",", ", ", ","))))
...
clean_studies %>%
  ...
  select(Study, Type = ExamType, `# Same Source` = SS_Tot, `# Diff Source` = DS_Tot, `Correct Source Decision Rate` = CSDR, `Overall Error Rate` = Overall_Error, `Inconclusive Rate` = Inconclusive_Rate) %>%
  mutate(across(c(`# Same Source`, `# Diff Source`), ~sprintf("% 5d", as.numeric(.)) %>% str_replace("NA", " ?"))) %>%
  mutate(across(c(`Correct Source Decision Rate` , `Overall Error Rate`, `Inconclusive Rate`), ~sprintf("%0.4f", .) %>% str_replace("NA", "?"))) %>%
...
As a result, it is difficult to evaluate it in a manner similar to the other experimental studies, and the definitions of drop-out and item non response rate provided above are consistent with experiments, not observational studies.
